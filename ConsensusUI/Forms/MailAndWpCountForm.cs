...
namespace ConsensusUI.Forms
{
    public partial class MailAndWpCountForm : XtraForm
    {
        ...
        private async void MailAndWpCountForm_Load(object sender, EventArgs e)
        {
            ...
            nmr_wp.Focus();
            DataTable dt = await SQLCrud.LoadDataIntoGridViewAsync("SELECT * FROM CustomersConsensus WITH (NOLOCK) WHERE CustomerToken='" + remoteSQL.Rows[0]["WebToken"].ToString() + "' AND CustomerPassword='" + remoteSQL.Rows[0]["WebPassword"].ToString() + "'", remoteSQL.Rows[0][0].ToString());
